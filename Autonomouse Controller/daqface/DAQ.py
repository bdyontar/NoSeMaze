...
class NiUsbDigitalOutTwoDevicesC:
    ...
    def __init__(self, device1, device2, secs, on1, on2):
        """
        Parameters
        ----------
        device1 : str
        ...
        sec : float
            Duration of the ventil should be opened. It is the water amount 
...
class DoAiConcatenatedPretrainingMultiTask:
    ...
    def __init__(self, ai_device, ai_channels, do_device, fv_device, 
                 ...
                 odor_write, fv_write, sync_clock, static, lick_channel_l, 
                 lick_channel_r, beam_channel, rewarded):
        """
        Parameters
        ----------
        ai_device : str
        ...
        self.ai_handle, self.do_handle, self.fv_handle = [TaskHandle(i) for i in range(3)]
        self.static = static
    ...
    def DoTask(self):
        ...
        if self.static:
            ...
            analogDataOnset = numpy.zeros((self.ai_channels, totalLength),dtype=numpy.float64)
            ...
            for i, length in enumerate(self.odours_length):
                if length > 0:
                    """before odor switch"""
                    totalLength = numpy.uint64(self.samp_rate*(length-self.onset))
                    analogDataBefore = numpy.zeros((self.ai_channels, totalLength), dtype=numpy.float64)
                    self.odor_pulse = numpy.sum([numpy.uint32([math.pow(2, i)]), self.valve8on], axis=0)
                    ...
                    if i != len(self.odours_length - 1):
                        """odor switch"""
                        self.odor_pulse = numpy.sum([numpy.uint32([math.pow(2, i+1)]), self.valve8on], axis=0)
                        totalLength = numpy.uint64(self.samp_rate*self.onset)
                        analogDataAfter = numpy.zeros((self.ai_channels, totalLength), dtype=numpy.float64)
                        DAQmxStartTask(self.do_handle)
                        DAQmxStartTask(self.ai_handle)
                        DAQmxWriteDigitalU32(self.do_handle, self.odorSampsPerChan, False, -1, DAQmx_Val_GroupByChannel, self.odor_pulse,
                                         byref(self.odorSampsPerChanWritten), None)
                    ...
                    analogDataOdor = numpy.hstack((analogDataBefore, analogDataAfter))
                    ...
                    analogDataOdourList = list()
                    analogDataOdor = numpy.zeros((self.ai_channels, 1), dtype=numpy.float64)
                    water_given = [False, False]
            
            if len(analogDataOdourList) == 0:
                analogData = analogDataOnset
            else:
                analogData = numpy.hstack((analogDataOnset, numpy.hstack(analogDataOdourList)))
    ...
    def CheckLicks(self, analogData, idx):
        """
        ...
        if self.reward_prob[0] > 0 and self.reward_prob[1] == 0:
            reward.deliver_reward_static(self.reward_device[0], self.water[0][idx])
            water_given[0][idx] = True
        elif self.reward_prob[0] == 0 and self.reward_prob[1] > 0:
            reward.deliver_reward_static(self.reward_device[1], self.water[1][idx])
...
class DoAiMultiTask:
    ...
    def __init__(self, ai_device, ai_channels, do_device, fv_device, 
                 reward_device_l, reward_device_r, samp_rate, secs, odor_write, 
                 fv_write, sync_clock, static, thorax_delay, lick_delay, 
                 lick_channel_l, lick_channel_r, beam_channel, rewarded):
        """
        Parameters
        ----------
        ai_device : str
...
#        print(self.static,self.thorax_delay,self.lick_delay,self.reward_device,self.reward_prob,self.water,self.lick_channel,self.beam_channel)
    ...
    def CheckLicks(self):
        ...
        check_l = numpy.hstack((self.analogDataOdorOn[self.lick_channel[0]], self.analogDataLickWindow[self.lick_channel[0]]))
        check_r = numpy.hstack((self.analogDataOdorOn[self.lick_channel[1]], self.analogDataLickWindow[self.lick_channel[1]]))
        
        check_beam = numpy.hstack((self.analogDataFvOnset[self.beam_channel], self.analogDataOdorOn[self.beam_channel], self.analogDataLickWindow[self.beam_channel]))
...
#            print('timestamp2')
            ...
            if self.odor_length > 0.01:
                ...
                self.analogDataOdorOn = numpy.zeros((self.ai_channels,1),dtype=numpy.float64)
            
            """lick window"""
            timestamp1 = datetime.datetime.now()
            if self.thorax_delay != 0:
                self.totalLength = numpy.uint64(self.samp_rate*self.thorax_delay)
                self.analogDataLickWindow = numpy.zeros((self.ai_channels,self.totalLength),dtype=numpy.float64)
                DAQmxCfgSampClkTiming(self.ai_handle, '', self.samp_rate, DAQmx_Val_Rising, DAQmx_Val_FiniteSamps,
                                      numpy.uint64(self.totalLength))
                DAQmxStartTask(self.ai_handle)
                ...
                self.analogDataLickWindow = numpy.zeros((self.ai_channels,1),dtype=numpy.float64)
            ...
            analogData = numpy.hstack((self.analogDataOnset, self.analogDataFvOnset, self.analogDataOdorOn, self.analogDataLickWindow, self.analogDataLick, self.analogDataOffset))
            shouldLickData = numpy.hstack((self.analogDataOdorOn, self.analogDataLickWindow, self.analogDataLick))
...
class DoAiMultiTaskWaitTraining:
    ...
    def CheckLicks(self):
        ...
        water_given = [False,False]
        if wait_licks_l + wait_licks_r == 0: #there is no water if not waited.
            if self.reward_prob[0] > 0 and self.reward_prob[1] == 0:
                if licks_l > 0 and licks_r == 0 and beam < 2:
                    if numpy.random.rand() < self.reward_prob[0]:
                        reward.deliver_reward_static(self.reward_device[0], self.water[0])
    ...
    def DoTask(self):
        ...
        if self.static:
            ...
            """saving analog data and free tasks"""
            analogData = numpy.hstack((self.analogDataOnset,self.analogDataFvOnset,self.analogDataOdorOn,self.analogDataLickWindow, self.analogDataLick, self.analogDataOffset))
            shouldLickData = numpy.hstack((self.analogDataLickWindow, self.analogDataLick))
...
class DoAiMultiTaskDirectTraining:
    """DAQ for normal GNG wait training. That is reward is always given after 
