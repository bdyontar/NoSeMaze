...
def show_risk_per_mouse(data: list[list[float]], mouse: str, bins: int) -> None:
    """
    ...
    fig.suptitle("Percent of Risk. ID: {}. \nN = {}, High Risk Probability = 0.4, Low Risk Probability = 0.8, Bin = {}".format(
        mouse, len(data[0]), bins))
    fig.autofmt_xdate()
...
def show_risk_per_odour(data: dict[str, list[list[float]]], mouses: list[str], bins: int) -> None:
    """
    ...
    fig.suptitle(
        "Percent of risk odour 2. \nHigh risk prob = 0.4, Low risk prob 0.8, Bin = {}.".format(bins))


def show_total_risk(data: dict[str, list[list[float]]], mouses: list[str], bins: int) -> None:
    """
    ...
    ax[0].set_title("ID: "+mouses[0] +
                    " Number of data = {}".format(len(data[mouses[0]][0])))
    ax[1].set_title("ID: "+mouses[1] +
                    " Number of data = {}".format(len(data[mouses[1]][0])))
    ...
    fig.suptitle(
        "Percent of sum risks odour 1 and odour 2. Bin = {}.".format(bins))


def show_percent_left_right(data: dict[str, list[list[float]]], mouses: list[str], bins: int) -> None:
    """
...
def show_percent_correct(data: dict[str, list[list[float]]], mouses: list[str], bins: int):
    """
...
def load_data(filepath: str, start_date: dt, stop_date: dt)->tuple[list[str], list[dict]]:
    ...
    for data in tmp:
        ...
        if "timestamp" in keys:
            tmp_dict["timestamp"] = dt.strptime(
                data["timestamp"], "%Y-%m-%d %H:%M:%S.%f")
        if "rewarded" in keys:
            tmp_dict["rewarded"] = dict(zip(["left prob", "right prob", "left reward", "right reward"], map(
                float, data["rewarded"].split("|"))))
        if "licks after odour" in keys:
            tmp_dict["licks after odour"] = dict(
                zip(["left", "right"], map(int, data["licks after odour"].split("|"))))
        ...
        if "licks at waiting" in keys:
            tmp_dict["licks at waiting"] = dict(
                zip(["left", "right"], map(int, data["licks at waiting"].split("|"))))
...
def get_performance(all_data: list[dict[str, list[list[float]]]], bins: int) -> list[list]:
    """
    ...
    for i, row in enumerate(all_data):
        if not_left_right(row):
            ...
            percent_correct = np.mean([d["correct"]
                                      for d in data if not_left_right(d)])*100
            right = [d["licks after odour"]["right"] >
                     0 for d in data if not_left_right(d) and d["correct"]]
            left = [d["licks after odour"]["left"] >
                    0 for d in data if not_left_right(d) and d["correct"]]
            ...
            number_high_risk_odour1 = sum([d["licks after odour"]["right"] > 0 for d in data if not_left_right(
                d) and d["rewarded"]["right prob"] <= 0.5 and d["correct"]])
            number_high_risk_odour2 = sum([d["licks after odour"]["left"] > 0 for d in data if not_left_right(
                d) and d["rewarded"]["left prob"] <= 0.5 and d["correct"]])

            number_low_risk_odour2 = sum([d["licks after odour"]["right"] > 0 for d in data if not_left_right(
                d) and d["rewarded"]["right prob"] > 0.5 and d["correct"]])
            number_low_risk_odour1 = sum([d["licks after odour"]["left"] > 0 for d in data if not_left_right(
                d) and d["rewarded"]["left prob"] > 0.5 and d["correct"]])
            ...
            if number_high_risk_odour1 == 0 and number_low_risk_odour1 == 0:
                ...
                percent_high_risk_odour1 = number_high_risk_odour1 / \
                    (number_high_risk_odour1 + number_low_risk_odour1) * 100
            ...
            if number_high_risk_odour2 == 0 and number_low_risk_odour2 == 0:
                ...
                percent_high_risk_odour2 = number_high_risk_odour2 / \
                    (number_high_risk_odour2 + number_low_risk_odour2) * 100
            ...
            if number_right == 0 and number_left == 0:
                ...
                percent_high_risk = (
                    number_high_risk_odour1 + number_high_risk_odour2) / (number_right + number_left) * 100
                percent_low_risk = (
                    number_low_risk_odour1 + number_low_risk_odour2) / (number_right + number_left) * 100

            performance.append([row["timestamp"], percent_correct, percent_left, percent_right, percent_high_risk_odour1,
                               percent_high_risk_odour2, percent_low_risk_odour1, percent_low_risk_odour2, percent_high_risk, percent_low_risk])
...
if __name__ == "__main__":
    ...
    for mouse in mouses:
        filepath = os.path.join(dropbox_path, dir_path,
                                "licks_log_"+mouse+".txt")
