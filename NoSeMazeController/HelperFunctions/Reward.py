...
def deliver_reward(do_device, samp_rate, secs, sync_clock, ai_channels):
    ...
    reward = daq.ThreadSafeDigitalOut(
        do_device, samp_rate, secs, vec, sync_clock, ai_channels)
    out = reward.DoTask()
...
def deliver_reward_static_concatenate(device1, device2, secs, index):
    ...
    on1 = 2**int(device1[-1])
    on2 = 2**int(device2[-1])
    reward = daq.NiUsbDigitalOutTwoDevicesC(
        device1, device2, secs[index], on1, on2)
    out = reward.DoTask()
...
def deliver_reward_static_two(device1, device2, secs1, secs2):
    ...
    on1 = 2**int(device1[-1])
    on2 = 2**int(device2[-1])
    reward = daq.NiUsbDigitalOutTwoDevices(
        device1, device2, secs1, secs2, on1, on2)
    out = reward.DoTask()
