...
def odor_sequence(odour_choice, n_trials): #Done randomisiertes Odour sequence
    sequence = np.empty(n_trials,dtype=int)
    length = len(odour_choice)
    rest = n_trials % length
    for t in range(int(n_trials/length)):
        sequence[t*length:(t+1)*length] = np.random.choice(odour_choice,length,replace=False)
    if rest != 0:
