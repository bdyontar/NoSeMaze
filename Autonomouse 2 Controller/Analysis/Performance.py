...
def show_risk_per_mouse(data, mouse, bins):
    ...
    fig.suptitle("Percent of Risk. ID: {}. \nN = {}, High Risk Probability = 0.4, Low Risk Probability = 0.8".format(mouse, len(data[0])))
    fig.autofmt_xdate()
...
def load_data(filepath, start_date, stop_date):
    """Load data. Return a list of headers 
    and a list of dictionary of all data with headers as keys."""
...
def get_performance(all_data, bins):
    """Data is a list of data. Bins is the window to determine performance.
    Return a list of dictionary of performance with the length of data."""
...
#            print(number_correct, number_left+number_right, number_left, number_right)
