...
#    table = ['Tag Id | Trials | Trials 12 Hrs | Wait 12 Hrs | Rewarded (R) 12 Hrs | Not-Rewarded (NR) 12 Hrs | Avg. Wait Licks R 12h | Avg. Wait Licks NR 12h | Avg. Response Licks R 12h | Avg. Response Licks NR 12h | Licks R 12h | Licks NR 12h | Trials R 12h | Trials NR 12h | Water Given 12 Hrs ']
    ...
    for animal in experiment.animal_list.keys():
        ...
        n_trials_last_12h = Analysis.n_trials_since(this_mouse, datetime.datetime.now() - datetime.timedelta(hours=12))
        ...
        if len(licks_list) != 0:
            ...
            for x in range(len(licks_T[0])):
                time_x = datetime.datetime.strptime(licks_T[0][x].strip('\n'),'%Y-%m-%d %H:%M:%S.%f')
                if time_x <= datetime.datetime.now() - datetime.timedelta(hours=12):
                    y = x
            if y != None:
                water_given = np.sum(list(map(float,licks_T[5][y:])))
            else:
                water_given = 0.0
        else:
            water_given = 0.0
        ...
        for schedule in this_mouse.schedule_list:
            ...
            if len(schedule.trial_list) == 0:
                analysis_table.append("{name} | 00.00 % | 00.00 % | 00.00 % | 00.00 % | NaN | NaN | 0 | 0\n".format(name=str(schedule.id)))
            elif left == 0:
                ...
                analysis_table.append("{name} | {percentl:.2f} | {percentr:.2f} | {percentg:.2f} | {percentrej:.2f} | {dprime1:.2f} | {dprime2:.2f} | {no_gng_l} | {no_gng_r}\n".format(name=str(schedule.id), percentl=percent_correct_left, percentr=percent_correct_right, percentg=percent_correct_rew_prob, percentrej=percent_correct_rejection,dprime1=float(dprime_lr), dprime2=float(dprime_gng), no_gng_l=num_rew_prob_l, no_gng_r=num_rew_prob_r))
        ...
        if os.path.isfile(experiment.logs_path+'/licks_log_'+animal+'.txt'):
            ...
            attachment.append(dict(log="".join(log), name='licks_log_'+animal+'.txt'))
        else:
            attachment.append(dict(log="The mouse hasn't licked...",name = 'licks_log_'+animal+'.txt'))
...
def warning_licks(logs_path, namelist):
    ...
    for name in namelist:
        if os.path.isfile(logs_path+'/licks_log_'+name+'.txt'):
            with open(logs_path+'/licks_log_'+name+'.txt','r') as f: log = f.readlines()
            if len(log)>100: log = log[-100:]
            attachment.append(dict(log="".join(log), name='licks_log_'+name+'.txt'))
        else:
