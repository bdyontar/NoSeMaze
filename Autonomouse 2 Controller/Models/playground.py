# -*- coding: utf-8 -*-
"""
Created on Fri Nov 16 11:12:16 2018
...
class Experiment:
    def __init__(self):
        ...
        self.last_data = None
        self.last_thorax_data = None
        self.default_licks = [['','','','']]
        self.licks = self.default_licks.copy()
        
    def update_licks(self, timestamp, time, total_licks, marker):
        if self.licks == self.default_licks.copy():
            self.licks[0] = [timestamp, time, total_licks, marker]
        else:
            self.licks.append([timestamp, time, total_licks, marker])
        
    def save_licks(self):
        name = self.name.split('.')[0]
        fname = self.save_path + '/licks_log_'+name+'.txt'
        
        with open(fname, 'w', newline = '') as fn:
            writer = csv.writer(fn)
            writer.writerows(self.licks)
